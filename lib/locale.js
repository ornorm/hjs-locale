...
export class Locale {
    ...
    static getDefault(fallback = null) {
        if (defaultLocale === null) {
            ...
            if (IS_WIN) {
                throw new TypeError("Not Supported platform");
            } else {
                let ret = exec("locale");
                let all = ret.split("\n");
                if (all) {
                    lang = all[0].replace("LANG=", "").replace(".UTF-8", "");
                } else {
                    lang = "en_EN";
                }
            }
...
export const CHINA = Locale.SIMPLIFIED_CHINESE;
export const PRC = Locale.SIMPLIFIED_CHINESE;
export const TAIWAN = Locale.TRADITIONAL_CHINESE;
